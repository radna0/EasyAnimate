...
def init_ocr_reader(root: str = "~/.cache/easyocr", device: str = "gpu"):
    ...
    # Ensure the device is either CPU or GPU
    if "xla" in device:
        device = "cpu"  # Force CPU for OCR on TPU
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Compute the text score of the middle frame in the videos."
    )
    parser.add_argument(
    ...
    with state.split_between_processes(video_path_list) as splitted_video_path_list:
        for i, video_path in enumerate(tqdm(splitted_video_path_list)):
            try:
                ...
                logger.warning(
                    f"Compute text score for video {video_path} with error: {e}."
                )
