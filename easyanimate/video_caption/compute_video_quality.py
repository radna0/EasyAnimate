...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Compute scores of uniform sampled frames from videos."
    )
    parser.add_argument(
        "--video_metadata_path",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The evaluation metric(s) for generated images.",
    )
    parser.add_argument(
        "--batch_size",
        type=int,
...
def main():
    ...
    if os.path.exists(args.saved_path):
        ...
        merged_df = video_metadata_df.merge(
            saved_metadata_df, on="video_path", how="outer", indicator=True
        )
        video_metadata_df = merged_df[merged_df["_merge"] == "left_only"]
        # Sorting to guarantee the same result for each process.
        video_metadata_df = video_metadata_df.iloc[
            index_natsorted(video_metadata_df["video_path"])
        ].reset_index(drop=True)
        if args.caption_column is None:
            video_metadata_df = video_metadata_df[[args.video_path_column]]
        else:
            video_metadata_df = video_metadata_df[
                [args.video_path_column, args.caption_column + "_x"]
            ]
            video_metadata_df.rename(
                columns={args.caption_column + "_x": args.caption_column}, inplace=True
            )
        logger.info(
    ...
    if args.frame_sample_method == "image":
        logger.warning(
            "Set args.num_sampled_frames to 1 since args.frame_sample_method is image."
        )
    ...
    logger.info(
        f"Drop {len(video_metadata_df) % state.num_processes} videos to ensure each process handles the same number of videos."
    )
    ...
    with state.split_between_processes(video_metadata_list) as splitted_video_metadata:
        ...
        for idx, batch in enumerate(tqdm(video_loader)):
            if len(batch) > 0:
                ...
                batch_frame = batch[
                    "sampled_frame"
                ]  # [batch_size, num_sampled_frames, H, W, C]
                batch_caption = None
                if args.caption_column is not None:
                ...
                if args.video_folder == "":
                    ...
                    saved_video_path_list = [
                        os.path.relpath(video_path, args.video_folder)
