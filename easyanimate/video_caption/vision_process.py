    ...
    if isinstance(ele["video"], str):
        ...
        if "nframes" in ele:
            ...
            max_frames = floor_by_factor(
                ele.get("max_frames", min(
                    FPS_MAX_FRAMES, video.size(0))), FRAME_FACTOR
            )
        ...
        if not (FRAME_FACTOR <= nframes and nframes <= video.size(0)):
            raise ValueError(
                f"nframes should in interval [{FRAME_FACTOR}, {video.size(0)}], but got {nframes}."
            )
...
def get_image(
    ...
    # Check if the input is a PIL Image directly
    if isinstance(image, Image.Image):
        image_obj = image
    # Check if the input is a URL (HTTP or HTTPS)
    elif image_url and (
    ...
    # Check if the input is a file path
        ...
        image_obj = Image.open(image_url[7:])
    # Check if the input is base64-encoded
        ...
        image_obj = Image.open(image_url) if image_url else None

    # Raise error if image is not valid
    if image_obj is None:
        raise ValueError(
            "Invalid image input: provide a valid URL, base64 string, local path, or PIL.Image."
        )
...
def get_prompt_cpu(message_obj):
...
def get_qwen_prep():
    ...
    def shard_output(output, mesh):
        xs.mark_sharding(
            ...
            xs.get_global_mesh(),
            _prepare_spmd_partition_spec(
                output["last_hidden_state"], shard_maximal=True
            ),
        )

    model.visual = FSDPv2(model.visual)
    model.model = FSDPv2(model.model, shard_output=shard_output)
    model.lm_head = FSDPv2(model.lm_head)
    model.eval()
...
def get_prompt_xla(model, processor, messages, batch_size=2):
    import torch_xla as xla

    def create_batches(messages, batch_size):
        inputs_batch = []
        for i in range(0, len(messages), batch_size):
            # messages
            cur_batch = messages[i: i + batch_size]
            texts = [
                processor.apply_chat_template(
                    message_obj, tokenize=False, add_generation_prompt=True
                )
                for message_obj in cur_batch
            ]
            image_inputs, video_inputs = process_vision_info(
                [message_obj for message_obj in cur_batch]
            )
            inputs = processor(
                text=texts,
                images=image_inputs,
    ...
    inputs_batch = create_batches(messages, batch_size)
    output_texts = []
    for index, inputs in enumerate(inputs_batch):
