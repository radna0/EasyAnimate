            ...
            if self.transformer.config.in_channels != self.vae.config.latent_channels:
                ...
                self.pipeline = (
                    EasyAnimatePipeline_Multi_Text_Encoder_Inpaint.from_pretrained(
                        ...
                        scheduler=scheduler_dict["Euler"].from_pretrained(
        ...
        try:
            if self.transformer.config.in_channels != self.vae.config.latent_channels:
                if generation_method == "Long Video Generation":
                    if validation_video is not None:
                        raise gr.Error(
                            f"Video to Video is not Support Long Video Generation now."
                        )
                    init_frames = 0
