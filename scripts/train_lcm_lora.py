...
def parse_args():
    ...
    parser.add_argument(
        "--train_text_encoder",
        action="store_true",
        help="Whether or not to train text encoder.",
    )
        ...
        def get_random_downsample_ratio(sample_size, is_image=True):
            def create_special_list(length):
                ...
                if length >= 2:
                    ...
                    special_list = [first_element] + [other_elements_value] * (
                        length - 1
                    )
                    return special_list
            ...
            if sample_size >= 1536:
                number_list = (
                    [1, 1.25, 1.5, 2, 2.5, 3]
                    + [args.image_sample_size / args.video_sample_size]
                    if is_image
                    else []
                )
                ...
                number_list = (
                    [1, 1.25, 1.5] + [args.image_sample_size / args.video_sample_size]
                    if is_image
                    else []
                )
            elif sample_size >= 512:
                number_list = (
                    [1] + [args.image_sample_size / args.video_sample_size]
                    if is_image
                    else []
                )
            else:
        ...
        def collate_fn(examples):
            ...
            if data_type == "image":
                random_downsample_ratio = (
                    ...
                    else get_random_downsample_ratio(args.image_sample_size)
                )
                ...
                if args.random_hw_adapt:
                    if args.training_with_video_token_length:
                        ...
                        random_downsample_ratio = get_random_downsample_ratio(
                            args.video_sample_size, is_image=False
                        )
                        batch_video_length = (
                            args.video_sample_n_frames + sample_n_frames_bucket_interval
                        )
            ...
            if epoch == first_epoch and step == 0:
                ...
                if args.train_mode != "normal":
                    ...
                    for idx, (
                        ...
                        save_videos_grid(
                            ref_pixel_value,
                            f"{args.output_dir}/sanity_check/ref_{gif_name[:10] if not text == '' else f'{global_step}-{idx}'}.gif",
                            rescale=True,
                        )
            ...
            with accelerator.accumulate(transformer3d):
                    ...
                    if args.training_with_video_token_length:
                        if (
                            ...
                            * pixel_values.size()[4]
                            ...
                            ref_pixel_values = torch.tile(
                                ref_pixel_values, (4, 1, 1, 1, 1)
                            )
                ...
                if config.get("enable_multi_text_encoder", False):
                    ...
                    with torch.no_grad():
                        ...
                        un_prompt_embeds, un_prompt_attention_mask = encode_prompt(
                            tokenizer,
                            text_encoder,
                            ["" for _ in batch["text"]],
                            latents.device,
                        ...
                        un_prompt_embeds_2, un_prompt_attention_mask_2 = encode_prompt(
                            tokenizer_2,
                            text_encoder_2,
                            ["" for _ in batch["text"]],
                            latents.device,
                    ...
                    with torch.no_grad():
                        with torch.autocast(accelerator.device.type):
                            ...
                            uncond_teacher_output = teacher_transformer3d(
                                ...
                                clip_encoder_hidden_states=(
                                    torch.zeros_like(clip_encoder_hidden_states)
                                    if args.train_mode != "normal"
                                    else None
                                ),
                                clip_attention_mask=(
                                    torch.zeros_like(clip_attention_mask)
                                    if args.train_mode != "normal"
                                    else None
                                ),
                                return_dict=False,
                            )[0]
                            uncond_teacher_output, _ = uncond_teacher_output.chunk(
                                2, dim=1
                            )
