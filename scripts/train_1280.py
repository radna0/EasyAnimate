...
def _get_2d_rotary_pos_embed_cached(embed_dim, crops_coords, grid_size):
    ...
    print(
        "embed_dim=%d crops_coords=%s grid_size=%s in_cache=%d cache_size=%d"
        % (
            embed_dim,
            crops_coords,
            grid_size,
            tmp_key in rotary_pos_embed_cache,
            len(rotary_pos_embed_cache),
        )
    )
    ...
    try:
        ...
        if config.get("enable_multi_text_encoder", False):
            if args.train_mode != "normal":
                ...
                pipeline = (
                    EasyAnimatePipeline_Multi_Text_Encoder_Inpaint.from_pretrained(
                        args.pretrained_model_name_or_path,
...
def main():
    ...
    if args.enable_bucket:
        aspect_ratio_sample_size = {
            key: [x / 1280 * args.video_sample_size for x in ASPECT_RATIO_1280[key]]
            for key in ASPECT_RATIO_1280.keys()
        }
        aspect_ratio_random_crop_sample_size = {
            key: [
                x / 1280 * args.video_sample_size
                for x in ASPECT_RATIO_RANDOM_CROP_1280[key]
            ]
            for key in ASPECT_RATIO_RANDOM_CROP_1280.keys()
        }
        ...
        if args.keep_all_node_same_token_length:
            if args.token_sample_size > 256:
                ...
                numbers_list = [256]
            numbers_list = [
                _number * _number * args.video_sample_n_frames
        ...
        def get_length_to_frame_num(token_length):
            if args.image_sample_size > args.video_sample_size:
                sample_sizes = list(
                    range(args.video_sample_size, args.image_sample_size + 1, 128)
                )
            ...
            length_to_frame_num = {
                sample_size: min(
                    token_length / sample_size / sample_size, args.video_sample_n_frames
                )
                // sample_n_frames_bucket_interval
                * sample_n_frames_bucket_interval
        ...
        def collate_fn(examples):
            ...
            if data_type == "image":
                random_downsample_ratio = (
                    ...
                    else get_random_downsample_ratio(
                ...
                aspect_ratio_sample_size = {
                    key: [
                        x / 1280 * args.image_sample_size / random_downsample_ratio
                        for x in ASPECT_RATIO_1280[key]
                    ]
                    for key in ASPECT_RATIO_1280.keys()
                }
                aspect_ratio_random_crop_sample_size = {
                ...
                if args.random_hw_adapt:
                    if args.training_with_video_token_length:
                            ...
                            np.array(
                                [
                                    np.mean(
                                        np.array(
                                            [
                                                np.shape(example["pixel_values"])[1],
                                                np.shape(example["pixel_values"])[2],
                                            ]
                                        )
                                    )
                        ...
                        choice_list = [
                            length
                            for length in list(length_to_frame_num.keys())
                            if length < local_min_size * 1.25
                        ]
                        ...
                        batch_video_length = length_to_frame_num[
                            local_video_sample_size
                        ]
                        random_downsample_ratio = (
                            args.video_sample_size / local_video_sample_size
                        )
                    else:
                ...
                aspect_ratio_sample_size = {
                    key: [
                        x / 1280 * args.video_sample_size / random_downsample_ratio
                        for x in ASPECT_RATIO_1280[key]
                    ]
                    for key in ASPECT_RATIO_1280.keys()
                }
            ...
            for example in examples:
                ...
                if args.train_mode != "normal":
                    ...
                    clip_index = get_random_clip_index(
        ...
        for step, batch in enumerate(train_dataloader):
                ...
                if args.train_mode != "normal":
                    ...
                    for idx, (clip_pixel_value, pixel_value, text) in enumerate(
                        ...
                        Image.fromarray(np.uint8(clip_pixel_value)).save(
                            f"{args.output_dir}/sanity_check/clip_{gif_name[:10] if not text == '' else f'{global_step}-{idx}'}.png"
                        )
                        save_videos_grid(
                            pixel_value,
                            f"{args.output_dir}/sanity_check/mask_{gif_name[:10] if not text == '' else f'{global_step}-{idx}'}.gif",
                            rescale=True,
                        )
            ...
            with accelerator.accumulate(transformer3d):
                ...
                if args.training_with_video_token_length:
                    if (
                        ...
                        * pixel_values.size()[4]
                        ...
                        if args.enable_text_encoder_in_dataloader:
                            if config.get("enable_multi_text_encoder", False):
                                batch["prompt_embeds"] = torch.tile(
                                    batch["prompt_embeds"], (4, 1, 1)
                                )
                                batch["prompt_attention_mask"] = torch.tile(
                                    batch["prompt_attention_mask"], (4, 1)
                                )
                                batch["prompt_embeds_2"] = torch.tile(
                                    batch["prompt_embeds_2"], (4, 1, 1)
                                )
                                batch["prompt_attention_mask_2"] = torch.tile(
                                    batch["prompt_attention_mask_2"], (4, 1)
                                )
                            else:
                                batch["encoder_hidden_states"] = torch.tile(
                                    batch["encoder_hidden_states"], (4, 1, 1)
                                )
                                batch["encoder_attention_mask"] = torch.tile(
                                    batch["encoder_attention_mask"], (4, 1)
                                )
                        else:
                        ...
                        * pixel_values.size()[4]
                        ...
                        if args.enable_text_encoder_in_dataloader:
                            if config.get("enable_multi_text_encoder", False):
                                batch["prompt_embeds"] = torch.tile(
                                    batch["prompt_embeds"], (2, 1, 1)
                                )
                                batch["prompt_attention_mask"] = torch.tile(
                                    batch["prompt_attention_mask"], (2, 1)
                                )
                                batch["prompt_embeds_2"] = torch.tile(
                                    batch["prompt_embeds_2"], (2, 1, 1)
                                )
                                batch["prompt_attention_mask_2"] = torch.tile(
                                    batch["prompt_attention_mask_2"], (2, 1)
                                )
                            else:
                                batch["encoder_hidden_states"] = torch.tile(
                                    batch["encoder_hidden_states"], (2, 1, 1)
                                )
                                batch["encoder_attention_mask"] = torch.tile(
                                    batch["encoder_attention_mask"], (2, 1)
                                )
                        else:
                ...
                if args.train_mode != "normal":
                    ...
                    mask_pixel_values = batch["mask_pixel_values"].to(
                    ...
                    if args.training_with_video_token_length:
                        if (
                            ...
                            * pixel_values.size()[4]
                            ...
                            clip_pixel_values = torch.tile(
                                clip_pixel_values, (4, 1, 1, 1)
                            )
                            mask_pixel_values = torch.tile(
                                mask_pixel_values, (4, 1, 1, 1, 1)
                            )
                            ...
                            * pixel_values.size()[4]
                            ...
                            clip_pixel_values = torch.tile(
                                clip_pixel_values, (2, 1, 1, 1)
                            )
                            mask_pixel_values = torch.tile(
                                mask_pixel_values, (2, 1, 1, 1, 1)
                            )
                ...
                if args.train_mode != "normal":
                    ...
                    t2v_flag = torch.from_numpy(np.array(new_t2v_flag)).to(
                        accelerator.device, dtype=weight_dtype
                    )
                    ...
                    if args.train_mode != "normal":
                        if vae.quant_conv.weight.ndim == 5:
                            ...
                            for i in range(0, mask_pixel_values.shape[0], bs):
                                ...
                                mask_pixel_values_bs = vae.encode(mask_pixel_values_bs)[
                                    0
                                ]
                        ...
                        inpaint_latents = (
                            t2v_flag[:, None, None, None, None] * inpaint_latents
                        )
                ...
                if args.enable_text_encoder_in_dataloader:
                    if config.get("enable_multi_text_encoder", False):
                        ...
                        prompt_embeds_2 = batch["prompt_embeds_2"].to(
                            accelerator.device, dtype=weight_dtype
                        )
                        prompt_attention_mask_2 = batch["prompt_attention_mask_2"].to(
                            accelerator.device, dtype=weight_dtype
                        )
                    else:
                ...
                if args.noise_share_in_frames:
                    ...
                    noise = generate_noise(
                        *latents.size(),
                        ratio=args.noise_share_in_frames_ratio,
                ...
                if config.get("enable_multi_text_encoder", False):
                    ...
                    if args.motion_sub_loss and noise_pred.size()[2] > 2:
                        ...
                        sub_loss = F.mse_loss(
                            gt_sub_noise, pre_sub_noise, reduction="mean"
                        )
                        loss = (
                            loss * (1 - args.motion_sub_loss_ratio)
                            + sub_loss * args.motion_sub_loss_ratio
                        )
                else:
